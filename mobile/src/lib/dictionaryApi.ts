...
export async function getWordDefinition(word: string): Promise<WordDefinition | null> {
  ...
  try {
    ...
    if (Array.isArray(data) && data.length > 0) {
      ...
      await cacheDefinition(normalizedWord, definition);
...
export async function getCachedDefinition(word: string): Promise<WordDefinition | null> {
  try {
...
async function cacheDefinition(word: string, definition: WordDefinition): Promise<void> {
  try {
